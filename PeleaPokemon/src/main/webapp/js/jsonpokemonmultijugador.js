window.onload = function(){
	...
	//socket.close();
		...
		console.log(datos);
									...
									let aux = Number(lobbieId)+1;
							
							    var socket2  = new WebSocket("ws://192.168.43.110:8080/ProyectoPokemon/websocket" + aux);
							    
							    terminarPartida.addEventListener("click",function(){
		socket2.send("terminar");
    });
    ...
    function ataque2f(){
        if(!tuTurno&& !terminado){
            accion("atacar2");
            socket2.send("atacar2");
        }
    }
    
    defensa1.addEventListener("click",function(){
        if(tuTurno&& !terminado){
            socket2.send("defensa1");
        }
    });
